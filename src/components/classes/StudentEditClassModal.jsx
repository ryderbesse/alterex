...
import { Upload, X, Book, FlaskConical, Monitor, Calculator, Palette as PaletteIcon, Music, Globe, Atom, PenTool, Languages, History, Heart } from 'lucide-react';
...
export default function StudentEditClassModal({ classData, onClose, onSave }) {
  const [color, setColor] = useState(classData?.color || 'violet');
  const [backgroundPattern, setBackgroundPattern] = useState(classData?.background_pattern || 'none');
  const [backgroundImage, setBackgroundImage] = useState(classData?.background_image || '');
  const [backgroundType, setBackgroundType] = useState(
    classData?.background_image ? 'image' : 
    classData?.color?.includes('-') ? 'gradient' : 'color'
  );
  const [gradesEnabled, setGradesEnabled] = useState(classData?.grades_enabled || false);
  const [targetGrade, setTargetGrade] = useState(classData?.target_grade || 90);
  const [gradeWeights, setGradeWeights] = useState(classData?.grade_weights || defaultWeights);
  ...
  const handleSave = async () => {
    try {
      ...
      await base44.entities.Class.update(classData.id, updates);
      toast.success('Class style updated');
      onSave();
      onClose();
    } catch (err) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Class Style</DialogTitle>
        </DialogHeader>
